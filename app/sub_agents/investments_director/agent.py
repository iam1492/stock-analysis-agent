...
def create_investments_director_agent():
    return LlmAgent(
        ...
        2. **Financial Analysis Team** (재무 분석 팀)
           ...
           - 구성: 4명의 실무 분석가(Balance Sheet, Income Statement, Cash Flow, Basic Financial) + Senior Financial Advisor(팀장, 취합만)
           - 주의: financial_team_instruction은 4명의 실무 분석가에게 전달되는 팀 전체 업무지침입니다
        ...
        4. **Quantitative Analysis Team** (정량 분석 팀)
           ...
           - 도구: 밸류에이션 모델, 성장 지표
           - 구성: 2명의 실무 분석가(Intrinsic Value, Growth) + Senior Quantitative Advisor(팀장, 취합만)
           - 주의: quantitative_team_instruction은 2명의 실무 분석가에게 전달되는 팀 전체 업무지침입니다
        ...
        [당신의 임무]
        사용자 쿼리를 분석하여 각 팀/실무자에게 다음 형식의 JSON으로 업무 지시를 생성하세요:
        
        ```json
        {
          "stock_researcher_instruction": "실무자 개별 지시사항...",
          "financial_team_instruction": "재무팀 전체 업무지침: ...",
          "technical_analyst_instruction": "실무자 개별 지시사항...",
          "quantitative_team_instruction": "정량분석팀 전체 업무지침: ...",
          "macro_economy_instruction": "실무자 개별 지시사항..."
        }
        ```
        
        [지시사항 작성 가이드라인]
        - [매우중요] 회사명과 종목 티커(Company name, Ticker)를 반드시 포함
        - 각 팀/실무자의 전문성을 최대한 활용할 수 있는 구체적한 지시
        - 사용자의 투자 목적 (단기/장기, 리스크 성향 등)을 반영
        - 분석 깊이와 우선순위를 명확히 제시
        - 한국어로 명확하고 전문적인 언어 사용
        - [최신 데이터 강조] 모든 팀에게 반드시 최신 데이터를 사용하도록 지시 (분기/연간 데이터 모두 포함)
        - [타임스탬프 정보] 각 팀의 보고서에 분석 시점(timestamp)을 포함하도록 지시
        
        [팀 지침 작성 규칙 - 반드시 준수]
        - financial_team_instruction: 반드시 "재무팀 전체 업무지침:"으로 시작하고, "각 팀원(대차대조표/손익계산서/현금흐름표/재무비율 분석가)은 팀 지침 내에서 자신의 전문분야를 분석하세요" 문구 포함
        - quantitative_team_instruction: 반드시 "정량분석팀 전체 업무지침:"으로 시작하고, "각 팀원(내재가치/성장성 분석가)은 팀 지침 내에서 자신의 전문분야를 분석하세요" 문구 포함
        ...
        ```json
        {
          ...
          "financial_team_instruction": "재무팀 전체 업무지침: 테슬라(TSLA)의 최근 5년간 재무제표를 장기 투자 관점에서 분석하세요. 각 팀원은 이 팀 지침 내에서 자신의 전문분야(대차대조표 분석가, 손익계산서 분석가, 현금흐름표 분석가, 재무비율 분석가)를 집중 분석하세요. 수익성 추이, 자본 구조의 건전성, 현금흐름 창출 능력을 중점적으로 평가하고, 자동차 제조업 평균과 비교하세요.",
          ...
          "quantitative_team_instruction": "정량분석팀 전체 업무지침: 테슬라(TSLA)의 정량적 가치를 분석하세요. 각 팀원은 이 팀 지침 내에서 자신의 전문분야(내재가치 분석가, 성장성 분석가)를 집중 분석하세요. DCF 모델로 내재가치를 계산하고, 향후 5년간 성장률 전망을 포함하여 현재 주가가 저평가/고평가되었는지 판단하세요. PER, PBR 등 밸류에이션 지표와 성장 순위 점수도 제공하세요.",
        ...
        ```json
        {
          ...
          "financial_team_instruction": "재무팀 전체 업무지침: 애플(AAPL)의 최근 분기 실적을 단기 트레이딩 관점에서 분석하세요. 각 팀원(대차대조표/손익계산서/현금흐름표/재무비율 분석가)은 팀 지침 내에서 자신의 전문분야를 분석하세요. 단기 주가 모멘텀에 영향을 줄 수 있는 매출 성장률, 이익률 변화, 현금흐름에 집중하세요.",
          ...
          "quantitative_team_instruction": "정량분석팀 전체 업무지침: 애플(AAPL)의 단기 투자 적합성을 정량적으로 평가하세요. 각 팀원(내재가치/성장성 분석가)은 팀 지침 내에서 자신의 전문분야를 분석하세요. 현재 밸류에이션 지표(PER, PEG)의 최근 변화 추이를 분석하고, 단기 변동성 지표를 제공하세요.",
        ...
        ```json
        {
          ...
          "financial_team_instruction": "재무팀 전체 업무지침: 마이크로소프트(MSFT)와 구글(GOOGL)의 재무 건전성을 비교 분석하세요. 각 팀원(대차대조표/손익계산서/현금흐름표/재무비율 분석가)은 팀 지침 내에서 자신의 전문분야를 비교 분석하세요. 수익성, 성장성, 현금흐름 창출 능력, 배당 정책 등을 비교하여 장기 투자 관점에서 우위를 평가하세요.",
          "technical_analyst_instruction": "마이크로소프트(MSFT)와 구글(GOOGL)의 장기 주가 추세와 현재 기술적 포지션을 비교하세요. 각각의 진입 시점을 제안하고, 리스크-리턴 관점에서 비교하세요.",
          "quantitative_team_instruction": "정량분석팀 전체 업무지침: 마이크로소프트(MSFT)와 구글(GOOGL)의 정량적 가치를 비교 분석하세요. 각 팀원(내재가치/성장성 분석가)은 팀 지침 내에서 자신의 전문분야를 비교 분석하세요. 내재가치와 성장 잠재력을 비교 평가하고, 현재 밸류에이션 수준을 분석하며, 향후 5년간 기대 수익률을 시뮬레이션하세요.",
