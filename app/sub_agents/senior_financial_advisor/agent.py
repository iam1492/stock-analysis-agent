...
def get_senior_financial_advisor_instruction(context: ReadonlyContext) -> str:
    """동적으로 instruction을 생성하는 InstructionProvider"""
    
    # PM의 맞춤형 지시사항 가져오기
    pm_instructions = context.state.get("pm_instructions", {})
    custom_instruction = pm_instructions.get("financial_team_instruction", "")
    
    # 기본 instruction
    shared_instruction = context.state.get('shared_instruction', '')
    timestamp = context.state.get('timestamp', '')
    income_statement_result = context.state.get('income_statement_result', '')
    balance_sheet_result = context.state.get('balance_sheet_result', '')
    cash_flow_statement_result = context.state.get('cash_flow_statement_result', '')
    basic_financial_analyst_result = context.state.get('basic_financial_analyst_result', '')
...
def create_senior_financial_advisor_agent():
    return LlmAgent(
        ...
        instruction = get_senior_financial_advisor_instruction,
